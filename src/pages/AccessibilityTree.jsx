...
function DOMTreeNode({ node, depth = 0, searchTerm, onSelectNode }) {
  ...
  const analyzeAccessibility = (element) => {
    const issues = [];
...
function NodeInspector({ node }) {
  ...
  const ariaAttributes = element ? 
    Array.from(element.attributes).filter(attr => attr.name.startsWith('aria-')) : 
    [];
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {node.classes.map((cls, idx) => (
                  <Badge key={idx} className="bg-yellow-500/20 text-yellow-400 text-xs font-mono">
                    {cls}
                  </Badge>
                ))}
              </div>
          ...
          <CardContent className="space-y-2">
            {ariaAttributes.map((attr, idx) => (
              <div key={idx} className="flex justify-between text-sm">
                <span className="text-purple-400 font-mono">{attr.name}:</span>
      ...
      {computedStyle && (
        <Card className="bg-white/5 border-white/10">
          ...
          <CardContent className="space-y-1 text-xs">
            {['display', 'position', 'width', 'height', 'opacity', 'z-index'].map(prop => (
              <div key={prop} className="flex justify-between font-mono">
                <span className="text-slate-400">{prop}:</span>
                <span className="text-cyan-400">{computedStyle[prop]}</span>
              </div>
            ))}
          </CardContent>
        </Card>
...
export default function AccessibilityTree() {
  ...
  const buildDOMTree = (element, maxDepth = 10, currentDepth = 0) => {
  ...
  const scanDOM = () => {
    ...
    const withAria = document.querySelectorAll('[aria-label], [aria-labelledby], [role]');
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 mt-1">Inspect DOM structure and accessibility</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card className="bg-white/5 border-purple-500/20">
          <CardContent className="p-4 text-center">
            <p className="text-2xl font-bold text-purple-400">{stats.withAria}</p>
          ...
          <Card className="bg-white/5 border-white/10">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" onClick={scanDOM} variant="outline" className="border-white/10">
