...
import { Plus, X, Save, ArrowRight, GitBranch, Settings, RefreshCw, Activity } from 'lucide-react';
import { toast } from 'sonner';
import VisualWorkflowDesigner from './VisualWorkflowDesigner';
import SelfCorrectionConfig from './SelfCorrectionConfig';
...
export default function WorkflowBuilder({ workflow, onClose, onSave }) {
  ...
  const [formData, setFormData] = useState({
    ...
    self_correction: workflow?.self_correction || {
      enabled: false,
      max_corrections: 3,
      quality_threshold: 0.7,
      reroute_on_failure: true
    },
    feedback_config: workflow?.feedback_config || {
      ...
      <CardContent className="p-6">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-white/5 mb-6">
            <TabsTrigger value="basic" className="data-[state=active]:bg-purple-500/20 data-[state=active]:text-purple-400">
              <Settings className="w-4 h-4 mr-2" />
              Basic Info
            </TabsTrigger>
            <TabsTrigger value="visual" className="data-[state=active]:bg-blue-500/20 data-[state=active]:text-blue-400">
              <GitBranch className="w-4 h-4 mr-2" />
              Visual Designer
            </TabsTrigger>
            <TabsTrigger value="correction" className="data-[state=active]:bg-orange-500/20 data-[state=active]:text-orange-400">
          ...
          <TabsContent value="basic" className="space-y-6">
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="conditional">Conditional - Dynamic routing based on results</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <label className="text-sm text-slate-400">Workflow Steps (Quick Edit)</label>
                <Button
                  onClick={addStep}
                  size="sm"
              ...
              <p className="text-xs text-slate-500 mt-2">
                💡 Use the Visual Designer tab for drag-and-drop workflow creation
              </p>
            </div>
          </TabsContent>
          ...
          <TabsContent value="correction">
            <SelfCorrectionConfig
              config={formData.self_correction}
              onChange={(config) => setFormData({ ...formData, self_correction: config })}
            />
          </TabsContent>
        </Tabs>
